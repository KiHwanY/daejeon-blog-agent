...
├── tests/                  # 단위 테스트 (LLM·DB·네트워크 모두 모의, 89개)
...
- `tests/` 에 **89개** 케이스(5개 파일)가 있으며, **Claude API·DuckDuckGo·Pexels·PostgreSQL 호출을 전부 모의(mock)** 처리합니다. API 키나 실행 중인 DB 없이 `pytest`만으로 통과합니다.
