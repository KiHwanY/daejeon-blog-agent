...
- **Streamlit UI** — 주제/톤/SEO 입력, "리서치 근거 부족 시 생성 중단" 체크박스, "블로그 생성" 버튼, 단계별 진행 표시, 근거 부족 시 **빨간 경고 배너**, "에이전트 판단 로그" 접이식 패널(리서치 근거·검색 재시도·자체 검토 결과·재작성 여부·피드백), 출처 링크, 카드/초안 렌더링, `.md` 다운로드
...
결과를 `blog_posts`에 저장합니다. "에이전트 판단 로그" 패널에서 리서치 근거·검색 재시도·자체 검토 결과·재작성 여부·피드백을
...
4. **저장** — `critique_passed` · `critique_feedback` · `was_rewritten` · `search_retried` · `research_grounded` · `research_reason` 를 `blog_posts` 에 함께 저장합니다. 기존 글을 재사용할 때도 이 값들이 함께 조회되어 Streamlit "에이전트 판단 로그"와 경고 배너에 표시됩니다.
