...
**리서치 → 근거 확인 → 아웃라인 → 초안 → 자체 검토 → (필요하면) 재작성 → 대표 이미지** 순서로 한국어 블로그 글을 만들어 줍니다.

축제·맛집·행사처럼 지역 정보가 필요한 주제는 검색어와 프롬프트에 자동으로 "대전"을 반영하고, 대전시청·대전관광공사·
지역 언론 등 신뢰할 수 있는 지역 소스를 우선 참고하도록 설계되어 있습니다. 리서치로 검증 가능한 구체 정보를 충분히 얻지
못하면 사용자에게 경고하거나(옵션 B) 생성을 중단(옵션 A)합니다. 완성된 글은 임베딩 벡터·자체 검토 결과와 함께
...
- **리서치 근거 부족 감지** — 리서치가 끝나면 Claude가 노트·출처를 다시 보고 "검증 가능한 구체 정보(수치·고유명사·장소·날짜 등)를 충분히 확보했는지" 판정 (`research_grounded` / `research_reason` 저장). 부족하면 기본(옵션 B)은 "일반론 위주·사실 지어내기 금지" 제약을 걸고 초안을 만들되 **결과 화면에 눈에 띄는 경고 배너**를 표시하고, 체크박스(옵션 A)를 켜면 아예 생성을 **중단**. 판정 실패 시에는 경고를 띄우지 않는 쪽(통과)으로 처리
- **자체 검토(critique) · 재작성(rewrite)** — 초안 작성 직후 Claude가 사실 정확성·구조·문체(톤)·SEO 4개 기준으로 스스로 평가하고, 미흡하면 피드백을 반영해 **최대 1회** 다시 씀 (`critique_passed` / `critique_feedback` / `was_rewritten` 저장). 자세한 동작은 [아래](#에이전트-자체-판단-근거-확인--검토--재작성) 참고
...
- **Streamlit UI (카드 + 모달)** — 전체폭(wide) 레이아웃. 주제/톤/SEO 입력, "리서치 근거 부족 시 생성 중단" 체크박스, "블로그 생성" 버튼, 단계별 진행 표시, 근거 부족 시 **빨간 경고 배너**. 생성된 새 글·유사 글을 **3열 카드 그리드**(이미지 상단 / 제목·발췌·메타·⭐점수 하단)로 보여 주고, 카드의 **`🔎 상세 보기`** 버튼을 누르면 **모달**이 열려 대표 이미지·"에이전트 판단 로그"(리서치 근거·검색 재시도·자체 검토·재작성·피드백)·리서치/아웃라인/초안(또는 전체 본문)·SEO 체크·`.md` 다운로드를 스크롤하며 볼 수 있습니다. 모달은 `X`·바깥 클릭·`ESC`·`닫기` 버튼으로 닫히고 뒤 화면은 어둡게 딤 처리됩니다
...
│   └── test_generate_blog.py # generate_blog 파이프라인 (근거 부족 옵션 A/B 포함)
└── src/
    ├── config.py           # 공용 설정 상수 (모델 ID · 임계값 · 캐시 TTL · Pexels 키)
    ├── search_tools.py      # web_search 도구 + tool-use 루프 + 재시도 + 근거 판정 + JSON 추출 (공용)
    ├── research_agent.py    # 웹 검색 tool-use 리서치 에이전트 (독립 실행형 Q&A)
    ├── blog_agent.py        # 리서치→근거확인→아웃라인→초안→자체검토→재작성 파이프라인 + 유사 글 중복 방지
...
**블로그 생성**을 누르면, 유사한 기존 글이 있으면 그대로 보여 주고, 없으면
리서치 → 근거 확인 → 아웃라인 → 초안 → 자체 검토 → (필요 시) 재작성 → 이미지 검색 단계를 차례로 진행한 뒤
결과를 `blog_posts`에 저장합니다.

결과(새 글·유사 글)는 **3열 카드 그리드**로 표시됩니다. 카드마다 대표 이미지·제목·발췌·메타(톤·날짜)·⭐점수(유사도 또는 `🆕`)가
보이고, 카드의 **`🔎 상세 보기`** 버튼을 누르면 **모달**이 열립니다. 모달에는 대표 이미지, "에이전트 판단 로그"(리서치 근거·
검색 재시도·자체 검토 결과·재작성 여부·피드백), 새 글이면 리서치 결과 / 검색된 출처 / 아웃라인 / 초안 / SEO 키워드 체크,
유사 글이면 전체 본문, 그리고 `.md` 다운로드 버튼이 담기며 길면 스크롤됩니다. 모달은 오른쪽 위 `X` · 모달 바깥 클릭 · `ESC` ·
하단 `닫기` 버튼으로 닫히고, 열려 있는 동안 뒤 화면은 어둡게 딤 처리됩니다.

**리서치 근거 부족 시 생성 중단** 체크박스:

- **해제(기본, 옵션 B)** — 근거가 부족해도 초안을 만들되, "일반론 위주·사실 지어내기 금지" 제약을 걸고 결과 화면 상단에 빨간 경고 배너(`⚠️ 이 주제는 온라인에서 확인 가능한 구체적 정보가 부족합니다 …`)를 띄웁니다.
- **체크(옵션 A)** — 근거가 부족하면 아웃라인·초안·저장을 건너뛰고 즉시 중단, 경고 배너와 안내만 표시합니다.
...
```bash
# 블로그 파이프라인 (유사 글 확인 → 리서치 → 근거 확인 → 아웃라인 → 초안 → 자체 검토 → 저장)
...
## 에이전트 자체 판단 (근거 확인 · 검토 · 재작성)

### 0. 리서치 근거 확인 (`run_search_loop` / `_assess_grounding`)

리서치 tool-use 루프가 끝나면(정상 종료든 `max_turns` 폴백이든), Claude에게 리서치 노트와 출처 목록을 다시 보여주고
"이 주제로 블로그를 쓸 때 **검증 가능한 구체 정보**(수치·고유명사·장소·날짜·기관명 등)를 충분히 확보했는가"를 묻습니다.
응답은 `{"grounded": true/false, "reason": "..."}` JSON이며, 파싱·API 오류 시에는 **경고를 띄우지 않는 쪽(`grounded=True`)** 으로 처리합니다.

- `research_grounded` 가 `false` 인데 `generate_blog(abort_on_weak_research=False)`(기본, **옵션 B**)이면 →
  `write_draft` / `rewrite_draft` 에 "일반론 위주로만 쓰고, 리서치에 없는 구체 사실(수치·상호·주소·가격 등)은 절대 지어내지 말 것" 제약을 추가한 뒤 초안을 만들고, 결과·`blog_posts` 에 `research_grounded=False` + `research_reason` 을 담아 UI가 경고 배너를 띄웁니다.
- `abort_on_weak_research=True`(Streamlit 체크박스, **옵션 A**)이면 → 아웃라인·초안·저장을 건너뛰고 `{"aborted": True}` 로 즉시 반환합니다.
...
4. **저장** — `critique_passed` · `critique_feedback` · `was_rewritten` · `search_retried` · `research_grounded` · `research_reason` 를 `blog_posts` 에 함께 저장합니다. 기존 글을 재사용할 때도 이 값들이 함께 조회되어 상단 경고 배너와 카드 상세 모달의 "에이전트 판단 로그"에 표시됩니다.
...
- **Anthropic API는 사용량 기반 과금**입니다. "블로그 생성" 한 번에 리서치 tool-use 루프(최대 5턴 + 정리 1회) + 근거 판정 + 아웃라인 + 초안 + 자체 검토 + (재작성 시) 1회 + 이미지 키워드 변환으로 여러 번의 API 호출이 발생합니다. [console.anthropic.com](https://console.anthropic.com/)에서 사용량과 한도를 확인하고, 반복 테스트 시 비용에 유의하세요. (단위 테스트는 API를 호출하지 않습니다.)
